...
@Component({
  ...
  standalone: true,
  imports: [HttpClientModule], // Import necessary modules here
...
export class FriendCardComponent implements OnInit {
  ...
  addFriend(data: { senderId: number; receiverId: number }) {
    this.http
      .post(`${environment.BASE_URL}/follow-management/follow`, data, { headers })
      .subscribe(() => {
        this.isFriend = true;
  ...
  removeFriend(data: { senderId: number; receiverId: number }) {
    this.http
      .post(`${environment.BASE_URL}/follow-management/unfollow`, data, { headers })
      .subscribe(() => {
        this.isFriend = false;
