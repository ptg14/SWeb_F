...
@Component({
  ...
  standalone: true,
  imports: [FriendCardComponent, NgFor, HttpClientModule],
...
export class FriendsComponent implements OnInit {
  ...
  fetchUsersAndFriends() {
    ...
    this.http.get<{ body: any[] }>(`${environment.BASE_URL}/users`, { headers }).subscribe(usersResponse => {
      const users = usersResponse.body;

      this.http.get<{ body: any[] }>(`${environment.BASE_URL}/follow-management/following/${this.myId}`, { headers }).subscribe(followingResponse => {
